...
const alertDanger = (msj, name) => {
	...
	if (name == "SyntaxError") 
		mostrar.innerHTML = "No se encontro \"" + nombreBuscar
...
const llenarApiData = (data) => {
	...
	skill.innerHTML = JSON.stringify(data.abilities[0].ability.name).replaceAll('""', '');
	if (data.abilities.length > 1)
		skillHidden.innerHTML = JSON.stringify(data.abilities[1].ability.name).replaceAll('""', '');
	else 
...
const showDataList = () => {
	...
	if(nombreBuscar.value.length > 2) {
		fetch('https://pokeapi.co/api/v2/pokemon/?offset=0&limit=1100')
		.then(res => res.json())
		.then(data => {
			setDataList(data.results, nombreBuscar.value)
		})
		.catch(e => { 
			alertDanger(`${e.name}: ${e.message}`, `${e.name}`)
			console.log(new Error(e))
...
const setDataList = (data, nombreBuscar) => {
	const nombres = document.getElementById('nombres')
	const prueba = document.getElementById('prueba')
	for (var i = 0; i < data.length; i++) {
		const pokeName = JSON.stringify(data[i].name).slice(1, -1)
		if (pokeName.includes(nombreBuscar)) {
			nombres.innerHTML += "<option value = '" + pokeName + "'>"
		}
	}
}
